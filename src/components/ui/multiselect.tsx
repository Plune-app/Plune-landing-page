...
interface MultipleSelectorProps {
  ...
  OptionItem?: React.FC<{ label: string; value: string, avatar?: string }>
  DropdownMenuRole?: React.FC<{
          ...
          {selected.map((option) => {
            return (
              <div
                key={option.value}
                className={cn(
                  "animate-fadeIn bg-background text-secondary-foreground hover:bg-background relative inline-flex h-8 cursor-default items-center rounded-md border ps-3 pe-7 pl-2 text-xs font-medium transition-all disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 data-fixed:pe-2",
                  badgeClassName
                )}
                        ...
                        {dropdowns.map((option) => {
                          return (
                            ...
                            >
                              ...
                              {OptionItem ? <OptionItem label={option.label} value={option.value} /> : option.label}
                            </CommandItem>
                          )
                        })}
                      </>
